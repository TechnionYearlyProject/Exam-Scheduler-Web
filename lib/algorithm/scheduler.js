...
HeuristicCalculator.prototype.findBestDayForScheduling = function(course){
    ...
    for (var j = 0; j < this.examPeriodLength; j++){
        ...
        for (var i = 0; i < course.conflicts.length; i++){
            if (!course.conflicts[i].has_exam){
                continue;
            }
            let conflictId = course.conflicts[i].id;
            if (this.coursesExams[j].hasOwnProperty(course.conflicts[i].id)){
                let prepTime = course.conflicts[i].days_before || (course.conflicts[i].credit_point - 1);
                let daysBeforeConflict = this.coursesExams[j][conflictId];
...
HeuristicCalculator.prototype.updateHeuristic = function(course, indexOfAssignedExamDay, remove){
    ...
    if (indexOfAssignedExamDay == -1){
        ...
        this.examsAmountInADay[indexOfAssignedExamDay]++;
        for (var i = 0; i < this.coursesExams.length; i++){
            this.coursesExams[i][course.number] = -(indexOfExamDay) + i;
        }
    }
};
...
Schedule.prototype.assignExam = function(course, date){
    ...
    this.heuristicCalculator.updateHeuristic(course, index);
